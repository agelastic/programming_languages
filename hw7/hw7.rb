...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    ...
    elsif (not real_close(@x1,@x2) and @x1 > @x2) 
      then LineSegment.new(@x2,@y2,@x1,@y1)
    elsif (real_close(@x1,@x2) and not real_close(@y1,@y2) and @y1 > @y2)
      then LineSegment.new(@x2,@y2,@x1,@y1)
    else self
    end
  end
  ...
  # helper 1
  def real_close_intersect(aXstart,aYstart,aXend,aYend,bXstart,bYstart,bXend,bYend)
  ...
  # helper 2
  def not_real_close_intersect(aXstart,aYstart,aXend,aYend,bXstart,bYstart,bXend,bYend)
  ...
  # uses two helpers above to be more compact
  def intersectWithSegmentAsLineResult seg
    if real_close(@x1,@x2)
      then 
      if @y1 < seg.y1 then
        real_close_intersect(@x1,@y1,@x2,@y2,seg.x1,seg.y1,seg.x2,seg.y2)
      else 
        real_close_intersect(seg.x1,seg.y1,seg.x2,seg.y2,@x1,@y1,@x2,@y2)
      end
    else 
      if @x1 < seg.x1 then 
        not_real_close_intersect(@x1,@y1,@x2,@y2,seg.x1,seg.y1,seg.x2,seg.y2)
      else 
        not_real_close_intersect(seg.x1,seg.y1,seg.x2,seg.y2,@x1,@y1,@x2,@y2)
      end
    end
  end
end
