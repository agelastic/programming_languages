	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift(dx,dy,e1) => 
        case eval_prog (e1, env) of
          ...
          | LineSegment(xstart,ystart,xend,yend) => LineSegment(xstart + dx, ystart + dy, xend + dx, yend + dy)
          | _ => raise BadProgram("bad shift expression:") 
...
fun preprocess_prog (e) =
    case e of
      ...
      | LineSegment(xstart,ystart,xend,yend) => 
	if real_close_point (xstart,ystart) (xend, yend) then Point(xstart,ystart)
	else if not (real_close(xstart, xend)) andalso xstart > xend 
	then LineSegment(xend,yend,xstart,ystart)
	else if (real_close (xstart, xend) andalso not (real_close(ystart,yend)) andalso ystart > yend) 
	then LineSegment(xend,yend,xstart,ystart)
	else e
      | _ => e
